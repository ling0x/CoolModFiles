...
function Player({ sharedTrackId, backSideContent, latestId, sharedTrackUrl }) {
  ...
  const playMusic = (id) => {
    ...
    player.pause();
    player
      // TODO: Make module URL dynamic - should accept either trackId or
      // external URL as parameter depending on states
      // .load(`https://modarchive.org/jsplayer.php?moduleid=${id}`)
      .load(
        "/api/proxy-mod?url=http://modland.antarctica.no/pub/modules/Protracker/Radix/crisis%20inverted.mod",
      )
      .then((buffer) => {
        setLoading(false);
        player.play(buffer);
